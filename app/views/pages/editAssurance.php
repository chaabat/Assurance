...
<form class="mt-40 max-w-sm mx-auto " action="<?= URL_ROOT ?>/pages/editAssurance" method="post">
 
<h3 class="mb-8 text-lg font-bold dark:text-white">Modifier:  </h3>
 

    <div class="mb-5">
        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nom</label>
        <input value="<?php echo $data["assurence"]->nom ?>" type="text" name="nom" id="nom"
    ...
    <div class="mb-5">
        ...
        <input value="<?php echo $data["assurence"]->adress ?>" type="text" name="adress" id="adress"
            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
            required>
    </div>
    </div>
    <input type="hidden" name="id_assureur" rows="4" value="<?php echo $data["assurence"]->id_assureur
 ?>"
